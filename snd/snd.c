...
void    snd_fill(void *udata, uint_8 *stream, int len)
{
    ...
    if (len > 0)
    {
        ...
        {
          short* src_audio_buffer = snd->mixbuf.dirty[0];
          short* tgt_audio_buffer = sdl_sound_buffer;
          int index = 0;
          for (index = 0; index < (len >> 2); index++ ) {
            *tgt_audio_buffer++ = src_audio_buffer[index];
            *tgt_audio_buffer++ = src_audio_buffer[index];
          }
          SDL_MixAudio(stream, (char *)sdl_sound_buffer, len, volume);
        }
