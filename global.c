...
loc_intel_load_settings(char *chFileName)
{
  ...
  while (fgets(Buffer,512, FileDesc) != (char *)0) {
    ...
    *Scan = '\0';
    ...
    if (!strcasecmp(Buffer,"psp_joystick_cursor"))  INTEL.psp_joystick_cursor = Value;
    else
...
intel_save_paths()
{
  ...
  snprintf(FileName, MAX_PATH, "%s/global.conf", INTEL.intel_home_dir);

  fp = fopen(FileName, "w");

  if (fp != (FILE *)0 ) {
      fprintf(fp, "%s=%s\n", DEFAULT_ROM_PATH,INTEL.intel_rom_dir);
      fclose(fp);
...
intel_load_paths()
{
   ...
   snprintf(file_name, MAX_PATH, "%s/global.conf", INTEL.intel_home_dir);
   ...
   while ((read = getline(&line, &len, fp)) != -1) {
      ...
      if(options){
          if(strcmp(options[0], DEFAULT_ROM_PATH) == 0){
            strcpy(INTEL.intel_rom_dir, options[1]);
          }
      }
   }
...
# endif
  INTEL.psp_joystick_cursor = 0;
  INTEL.psp_active_joystick = 0;
  INTEL.psp_cpu_clock       = 0;
...
intel_global_init(void)
{
  ...
  strcpy(INTEL.intel_home_dir, ".");

#if defined(GCW0_MODE)
   static char *tmp_directory, *home_name;

   home_name = malloc(sizeof(char)*6 + 1);
   strcpy(home_name,".intel");

   sprintf(INTEL.intel_home_dir, "%s/%s", getenv("HOME"), home_name);
   mkdir(INTEL.intel_home_dir, 0777);
   ...
   strcpy(INTEL.intel_rom_dir, tmp_directory);
   mkdir(tmp_directory, 0777);
   free (tmp_directory);
...
intel_load_rom(char *FileName, int zip_format)
{
  ...
  if (! error ) {
    ...
    strcpy(INTEL.intel_rom_dir, file_path);
    intel_save_paths();
