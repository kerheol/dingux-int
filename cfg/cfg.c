...
# else
   ...
   sprintf(buffer, "%s/%s/bios", getenv("HOME"), home_name);
...
//  if (errno) perror("errno value");
    ...
    if (!f || file_read_rom16(f, 4096, top_intv.exec_img) != 4096)
    {
        ...
        fprintf(stderr, "ERROR:  Could not read EXEC image '%s'\n",
                top_intv.fn_exec);
        show_error_and_exit("Unable to load exec.bin image");
        exit(1);
    }
    fseek(f, 0, SEEK_END);
    ...
    if (!f || file_read_rom8 (f, 2048, top_intv.grom_img) != 2048)
    {
        ...
        fprintf(stderr, "ERROR:  Could not read GROM image '%s'\n",
                top_intv.fn_grom);
        show_error_and_exit("Unable to load grom.bin image");
        exit(1);
    }
    fclose(f);
    ...
    if (tmp == NULL)
    {
        ...
        show_error_and_exit("Unable to load game.bin image");
        exit(1);
    }
